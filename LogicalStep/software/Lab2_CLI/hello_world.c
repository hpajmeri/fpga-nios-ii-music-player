                ...
                */
				...
				while (p1)
				{
					...
					case 2:
						...
						break;
					case 3:
						//normal speed - mono in both channels
						for (i = 0; i < cnt; i += 4)
						{
							leftbuf = Buff[i+1] << 8 | Buff[i];
							while(alt_up_audio_write_fifo_space(audio_dev, ALT_UP_AUDIO_RIGHT) == 0){}
							while( alt_up_audio_write_fifo_space(audio_dev, ALT_UP_AUDIO_LEFT)==0){}
							alt_up_audio_write_fifo (audio_dev, &(leftbuf), 1, ALT_UP_AUDIO_RIGHT);
							alt_up_audio_write_fifo (audio_dev, &(leftbuf), 1, ALT_UP_AUDIO_LEFT);
						}
						break;
...
//						p1 = 0;
//					}
//					res = f_read(&File1, filebuf, cnt, &s2);
//	        			for (i = 0; i < cnt; i += 4)
//	        			{
//	        				leftbuf = filebuf[i+1] << 8 | filebuf[i];
//	        				rightbuf = filebuf[i+3] << 8 | filebuf[i+2];
//
//							// write audio buffer
...
//                for (i = 0; i < p1; i += 4)
//                {
////                	uint16_t leftbuf[2048];
////                	uint16_t rightbuf[2048];
//
//                	uint16_t leftbuf;
//                	uint16_t rightbuf;
//
//					memcpy(&leftbuf, filebuf+i, 2);
//                	memcpy(&rightbuf, filebuf+i+2, 2);
//
//
////                	int j;
////                	for (j=0; i<8192; j += 4)
////                	{
////                		memcpy(&leftbuf+j/4, filebuf+j, 2);
////                		memcpy(&rightbuf+j/4, filebuf+j+2, 2);
////                		i = j;
////                	}
////
////					// write audio buffer
////					alt_up_audio_write_fifo (audio_dev, &(rightbuf), 2048, ALT_UP_AUDIO_RIGHT);
////					alt_up_audio_write_fifo (audio_dev, &(leftbuf), 2048, ALT_UP_AUDIO_LEFT);
////
////					// read audio buffer
////					alt_up_audio_read_fifo (audio_dev, &(r_buf), 2048, ALT_UP_AUDIO_RIGHT);
////					alt_up_audio_read_fifo (audio_dev, &(l_buf), 2048, ALT_UP_AUDIO_LEFT);
//
//					// write audio buffer
//					alt_up_audio_write_fifo (audio_dev, &(rightbuf), 1, ALT_UP_AUDIO_RIGHT);
//					alt_up_audio_write_fifo (audio_dev, &(leftbuf), 1, ALT_UP_AUDIO_LEFT);
